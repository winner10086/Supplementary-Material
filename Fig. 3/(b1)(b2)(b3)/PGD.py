...
class PGD(Attack):
    ...
    def forward(self, images, labels, actual_image_transform_matrix, actual_image_matrix, mark_allow_tamper_pixel, degree):
        ...
        for _ in range(self.steps):
            ...
            adv_images[0][0] = torch.clamp(adv_images[0][0], min=-2.1179, max=2.2489).detach()
            adv_images[0][1] = torch.clamp(adv_images[0][1], min=-2.0357, max=2.4285).detach()
            adv_images[0][2] = torch.clamp(adv_images[0][2], min=-1.8044, max=2.64).detach()

            adv_image = adv_images.squeeze(0).cpu()
            adv_image = con_transform(actual_image_transform_matrix, adv_image, actual_image_matrix)
            adv_image = np.clip(adv_image, actual_image_matrix-degree, actual_image_matrix+degree)

            adv_image = np.where(mark_allow_tamper_pixel, adv_image, actual_image_matrix)
