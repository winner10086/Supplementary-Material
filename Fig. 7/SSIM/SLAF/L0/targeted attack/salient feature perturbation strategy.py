...
def mark_matrix(input_matrix):
    matrix = input_matrix
    unique_elements = np.unique(matrix)
    num_unique = len(unique_elements)
    threshold = np.percentile(unique_elements, 50)
    mark_matrix_actual_index = np.where(matrix > threshold, 1, 0)
    return mark_matrix_actual_index
...
def calculate_ssim_for_folders(folder1, folder2, start_ssim=0.99, step=0.002):
    ...
    num_points = 4
    ssim_thresholds = {f">{start_ssim + i * step:.3f}": 0 for i in range(num_points)}
    ...
    for file_name1, file_name2 in zip(file_list1, file_list2):
        if file_name1.endswith(('.png', '.jpg', '.jpeg', '.bmp')) and file_name2.endswith(
            ...
            try:
                ...
                print(f"SSIM for {file_name1} and {file_name2}: {ssim_value}")
                ...
                for i in range(num_points):
                    threshold = start_ssim + i * step
                    if ssim_value > threshold:
                        ssim_thresholds[f">{threshold:.3f}"] += 1

            except Exception as e:
                print(f"Failed to process {file_name1} and {file_name2}: {e}")
                continue

    average_ssim = total_ssim / len(ssim_results) if ssim_results else 0

    print("\nSSIM Threshold Statistics:")
    for threshold, count in ssim_thresholds.items():
        percentage = (count / total_images * 100) if total_images > 0 else 0
        print(f"{threshold}: {count} images ({percentage:.2f}%)")
    print(f"\nThe average SSIM is: {average_ssim}")

    return ssim_results, average_ssim

# Calculate the L2 distance of each pixel after averaging
def l2_balance(normalization_l2, pixel_total_num):
    return math.sqrt(normalization_l2 ** 2 * pixel_total_num / pixel_total_num)
...
# Set the initial SSIM
start_ssim = 0.976

# Set the SSIM step size
step = 0.005
...
for image_file in image_files:
    ...
    attack_index = target_index_image_path(actual_image_absolute_path, index_file_absolute_path, weight_file_absolute_path,
                                           model_current)
    print(f"The target attack category is：{attack_index}")

    gradient_matrix = calculate_gradient(actual_image_absolute_path, attack_index, weight_file_absolute_path, model_current)
    ...
    while flag == 0 and num_iterative <= max_num_iterative:
        ...
        iterative_image_top1_index, x = predict_image_path(iterative_image_path, index_file_absolute_path, weight_file_absolute_path, attack_index, model_current)

        iterative_image_top1_index, x = predict_image_path(iterative_image_path, index_file_absolute_path,
        ...
        ####################################################################################
        gradient_matrix = -1 * calculate_gradient(iterative_image_path, attack_index, weight_file_absolute_path, model_current)
        iterative_image_pixel_weight_matrix_in_attack_label_R = gradient_matrix[0]
        iterative_image_pixel_weight_matrix_in_attack_label_G = gradient_matrix[1]
        iterative_image_pixel_weight_matrix_in_attack_label_B = gradient_matrix[2]
        ...
        for i in range(224):
            for j in range(224):
                ...
                if mark_matrix_final_R[i][j] == 1:
                    if iterative_image_pixel_weight_matrix_in_attack_label_R[i][j] > 0:
                ...
                if mark_matrix_final_G[i][j] == 1:
                    if iterative_image_pixel_weight_matrix_in_attack_label_G[i][j] > 0:
                ...
                if mark_matrix_final_B[i][j] == 1:
                    if iterative_image_pixel_weight_matrix_in_attack_label_B[i][j] > 0:
        ...
        attack_image_index, x = predict_image_path(attack_image_path, index_file_absolute_path,
                                                   weight_file_absolute_path, attack_image_index, model_current)
        ...
        if attack_image_index == attack_index:
            ...
            iterative_image_path = f"{output_directory}\\{image_file}"
            shutil.copy(attack_image_path, iterative_image_path)
        else:
            iterative_image_path = f"{output_directory}\\{image_file}"
            shutil.copy(attack_image_path, iterative_image_path)

ssim_results, average_ssim = calculate_ssim_for_folders(actual_images_folder_absolute_path, output_directory, start_ssim, step)