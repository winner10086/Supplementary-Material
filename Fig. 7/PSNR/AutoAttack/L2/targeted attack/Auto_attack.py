...
def calculate_psnr_for_folders(folder1, folder2):
    ...
    for file_name1, file_name2 in zip(file_list1, file_list2):
        if file_name1.endswith(('.png', '.jpg', '.jpeg', '.bmp')) and file_name2.endswith(
            ...
            print(f"SSIM for {file_name1} and {file_name2}: {psnr_value}")

            psnr_results.append((file_name1, file_name2, psnr_value))
    ...
    print("\nASR:")
    for bin_name, count in psnr_bins.items():
        print(f"{bin_name}: {count*100/len(psnr_results)} images")
...
for file_name in file_list:
    ...
    attack = APGDAttack_targeted(model, norm='L2', device=device)
    attack.n_restarts = 1
    attack.n_iter = 100
...
psnr_results = calculate_psnr_for_folders(folder_path, save_path_for_adversarial_samples)