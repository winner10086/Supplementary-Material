"""
The primary function of this code file is to perform targeted attacks on a specified convolutional neural network using JSMA based on the L0 norm.
...
def caculate_PSNR(img1_path, img2_path):
...
def model_predict(image_path, weights_path, model_cnn):
    ...
    return top_indices[0], top_indices[499]
...
number_of_tampered_pixels_per_round = int(tampering_elements_max_num / iterations_max_num)
...
for file_name in file_list:
    ...
    actual_labei_Top_1 = current_label_top1
    print("Attack target category index：", target_label)
    img_path = actual_img_path
    img_path_copy = img_path
    while num_iterations < iterations_max_num and torch.sum(mark_matrix_total) >= 3 * 224 * 224 * (1 - ratio) and current_label_top1 != target_label:
        num_iterations = num_iterations + 1
        print("###########################################################")
        print(f"Current iteration count:{num_iterations}")
        ...
        adv_image = data_transform(Image.open(img_path))  # apply data transform to original image
        ...
        psnr_actual = caculate_PSNR(image_absolute_path, img_path)
        if psnr_actual < L_psnr:
            new_image_path = os.path.join(save_path_for_adversarial_samples, file_name)
            shutil.copy(img_path_copy, new_image_path)
            break
        else:
            image_rgb = np.stack([actual_image[0], actual_image[1], actual_image[2]], axis=-1)
            # Convert data type to 8-bit unsigned integer
            ...
            # Create PIL image object
            image_pil = Image.fromarray(image_rgb)
            image_pil.save("attack_Image_last.png")
            img_path_copy = "attack_Image_last.png"

        current_label_top1, current_label_top2 = model_predict(img_path, weights_path, model_current)
        print("Target category classification value：", model_predict_attack_index(img_path, weights_path, target_label, model_current))
        print(f"Iterative image top 1 category:{current_label_top1}")

        if current_label_top1 == target_label or num_iterations == iterations_max_num or current_label_top1 != target_label:
